...
import { ArrowLeft, QrCode, Loader2, Users, CheckCircle2 } from 'lucide-react';
...
export const UserPointTransferApply = () => {
  ...
  useEffect(() => {
    ...
    const scannerId = 'transfer-reader';
    setTimeout(() => {
  ...
  useEffect(() => {
    const loadRule = async () => {
      if (!code || !selectedTargetGroupId || isMock) {
        setRule(null);
        return;
      }
    ...
    loadRule();
  }, [code, selectedTargetGroupId]);

  const handleApply = async () => {
    if (!userId || !code || !selectedTargetGroupId) return;
    setIsApplying(true);
    setMessage(null);
    ...
    try {
      if (isMock) {
        ...
        const sourceMembership = await db.userMemberships.where({ userId, groupId: mockCode.sourceGroupId }).first();
        if (!sourceMembership) {
        ...
        const targetLocal = await db.userMemberships.where({ userId, groupId: selectedTargetGroupId }).first();
        if (targetLocal?.id) {
      ...
      const sourceLocal = await db.userMemberships.where({ userId, groupId: codeData.source_group_id }).first();
      if (sourceLocal?.id) {
  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-main p-6 pb-24 font-sans">
      <div className="flex items-center mb-6">
        <Link to="/user/transfer" className="mr-4 p-2 bg-white rounded-full border border-gray-100 shadow-sm hover:bg-gray-50 transition">
      ...
      {activeTab === 'scan' && (
        <div className="animate-fade-in mb-6">
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div id="transfer-reader" className="w-full rounded-lg overflow-hidden"></div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-md mx-auto space-y-4">
        ...
        <button
          onClick={handleApply}
          disabled={isApplying || !code || !selectedTargetGroupId}
          className="w-full bg-primary text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-500/20 hover:bg-primary-dark transition disabled:opacity-50"
        >
          {isApplying ? <Loader2 className="animate-spin mx-auto" /> : '引き継ぎを実行'}
        </button>
      </div>
    </div>
  );
